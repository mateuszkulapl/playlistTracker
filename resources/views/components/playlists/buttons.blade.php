...
<div class="flex flex-col align-middle h-full items-stretch justify-center">
    ...
    <div class="grid grid-cols-1 gap-1 lg:grid-cols-2 lg:gap-2 p-1">
        ...
        <x-playlists.button class="text-blue-700 border-blue-700 hover:bg-blue-800 bg-blue-200 shadow-blue-500/50 focus:ring-blue-300bg-blue-700" href="{{ $playlist->getLink() }}" target="_blank">Otwórz</x-playlists.button>


        @if ($playlist->watchedAt == null)
            <x-playlists.button class=" duratext-green-700 border-green-700 hover:bg-green-800 bg-green-200 shadow-green-500/50 focus:ring-green-300bg-green-700" wire:click="watch('{{ $playlist->id }}')">Ukończ</x-playlists.button>

            @if ($playlist->inprogress != true)
                <x-playlists.button class="text-violet-700 border-violet-700 hover:bg-violet-800 bg-violet-200 shadow-violet-500/50 focus:ring-violet-300bg-violet-700" wire:click="setProgress('{{ $playlist->id }}')">W trakcie</x-playlists.button>
            @else
                <x-playlists.button class="text-violet-700 border-violet-700 hover:bg-violet-800 bg-violet-200 shadow-violet-500/50 focus:ring-violet-300bg-violet-700" wire:click="setUnprogress('{{ $playlist->id }}')">Przerwij</x-playlists.button>
            @endif
        @endif
        <x-playlists.button class="text-red-700 border-red-700 hover:bg-red-800 bg-red-200 shadow-red-500/50 focus:ring-red-300bg-red-700" wire:click="watch('{{ $playlist->id }}')">Usuń</x-playlists.button>
    </div>
</div>
