<div class="mb-1 pt-2" style="order:{{ $order }}">
    <form class="flex flex-row items-start justify-start gap-2" id="nowaNagroda" wire:submit.prevent="submit">
        ...
        <div class="flex flex-col">
            <input class="py-1 px-1 lg:px-2 w-24 border border-gray-300 rounded-sm text-sm focus:border-blue-500 focus:ring-blue-500" type="number" step="0.01" min="0" max="100" placeholder="Procent" wire:model="percentage" wire:loading.attr="disabled" wire:target="submit">
            @error('percentage')
                <span class="text-red-400">{{ $message }}</span>
            @enderror
        </div>
